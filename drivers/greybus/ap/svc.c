...
static uint8_t gb_svc_intf_hot_unplug(struct gb_operation *op)
{
    ...
    dev_info(&op->greybus->device, "interface %u unplugged\n", req->intf_id);
...
static struct gb_operation_handler gb_svc_handlers[] = {
    ...
    GB_HANDLER(GB_SVC_TYPE_INTF_HOT_UNPLUG, gb_svc_intf_hot_unplug),
};
